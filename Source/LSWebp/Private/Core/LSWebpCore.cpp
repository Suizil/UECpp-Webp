...
bool FLSWebpCore::GenerateStaticWebpPicture(FString& InPicturePath,
	...
	int32           InQualityFactor /*= 100*/)
{
	...
	{
		UE_LOG(LogLSWebp, Display, TEXT("FLSWebpCore::GenerateStaticWebpPicture Wrong ColorsNum"));
		return false;
	}
	...
	bool Result = FLSWebpLibStruct::GenerateWebpByRGBA(OutPicturePath,
		Data,
		InPictureSize.X,
		InPictureSize.Y,
		InQualityFactor);
...
bool FLSWebpCore::GenerateDynamicWebpPicture(FString& InPicturePath, TSharedPtr<FLSWebpPictureInformation> InWebpPictureInformation, TArray<TArray<FColor>>& InPicturesColors, TArray<int32>& WebpTimestepMillisecond, int32 InQualityFactor /*= 100*/)
{
	if (!CheckWebpPicturePath(InPicturePath))
	{
		return false;
	...
	bool Result = FLSWebpLibStruct::GenerateDymaicWebpByRGBA(
		OutPicturePath,
		Datas,
		Timestamps,
...
bool FLSWebpCore::LoadDynamicWebpPicture(FString InWebpFilePath, TArray<int32>& OutWebpTimestepMillisecond, TArray<TArray<FColor>>& OutPicturesColors,int32& OutWebpWidth,int32& OutWebpHeight)
{
	...
	bool bLoadWebp = FLSWebpLibStruct::LoadDynamicWebpPictureByRGBA(InWebpFilePath_ANSI, OutRGBADatas, OutTimestamps_ms, OutWebpWidth, OutWebpHeight);
...
bool FLSWebpCore::GetViewportSize(UObject* WorldContextObject, FVector2D& OutViewportSize)
{
...
bool FLSWebpCore::CheckWebpPicturePath(FString& InGeneratedWebpPicturesPath)
{
	...
	UE_LOG(LogLSWebp, Warning, TEXT("FLSWebpCore::CheckWebpPicturePath Wrong Path"));
	return false;
}

bool FLSWebpCore::CheckInWebpPictureSize(UObject* WorldContextObject, TSharedPtr<FLSWebpPictureInformation> InWebpPictureInformation)
{
	if (!WorldContextObject || !WorldContextObject->GetWorld())
	{
		return false;
	...
	UE_LOG(LogLSWebp, Warning, TEXT("FLSWebpCore::CheckInWebpPictureSize Wrong WebpPictureSize"));
	return false;
}
